...
struct NearLosslessMatrices {
    ...
    public mutating func predictMatrices(with predictor: ValuePredictor, options: EncodingOptions) {
        predictFirstPixel(options: options)
        predictFirstColumn(options: options, predictor: predictor)
        predictFirstRow(options: options, predictor: predictor)
        ...
        for i in 1..<self.width {
            for j in 1..<self.height {
                ...
                setPrediction(options: options, row: i, column: j, prediction: prediction)
            }
        }
    }
    
    private mutating func predictFirstPixel(options: EncodingOptions) {
        setPrediction(options: options, row: 0, column: 0, prediction: 128)
    }
    
    private mutating func predictFirstColumn(options: EncodingOptions, predictor: ValuePredictor) {
        let b: UInt8 = 0
        let c: UInt8 = 0
        ...
        for i in 1..<self.width {
            ...
            let prediction = predictor.predict(values: [a, b, c])
            setPrediction(options: options, row: i, column: 0, prediction: prediction)
        }
    }
    
    private mutating func predictFirstRow(options: EncodingOptions, predictor: ValuePredictor) {
        let a: UInt8 = 0
        let c: UInt8 = 0
        ...
        for j in 1..<self.height {
            ...
            let prediction = predictor.predict(values: [a, b, c])
            setPrediction(options: options, row: 0, column: j, prediction: prediction)
        }
    }
    
    private mutating func setPrediction(options: EncodingOptions, row: Int, column: Int, prediction: UInt8) {
        let k = options.acceptedError
